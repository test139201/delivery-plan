...
var GanttRenderer = (function () {
  ...
  /* ── 颜色工具 ── */
  var BLOCKER_COLORS = {
    '1': '#d97706', '2': '#be185d', '3': '#dc2626', '4': '#7c3aed',
    '5': '#2563eb', '6': '#0891b2', '7': '#059669', '8': '#64748b'
  };
  ...
  function renderGantt(config, containerId) {
    ...
    config.blockers.forEach(function (b, idx) {
      ...
      b.tasks.forEach(function (task) {
        ...
        /* Handle special crit segment for #2 → #3 (feed) */
        ...
        if (task.tags.waitFor) {
          ...
          /* Parse label: "等 #1 数据到位" → "等 " + badge + " 数据到位" */
    ...
    cutover.innerHTML = '<svg viewBox="0 0 6 1" preserveAspectRatio="none"><line class="cut-halo" x1="3" y1="0" x2="3" y2="1" vector-effect="non-scaling-stroke"/><line class="cut-line" x1="3" y1="0" x2="3" y2="1" vector-effect="non-scaling-stroke"/></svg><div class="cut-dot"></div><span>割接 W' + (config.cutover - 2) + '</span>';
  ...
  function bindEngine(containerId) {
    ...
    var onDocMove = function (e) {
      ...
      outD[d.bn].forEach(function (od) {
        ...
        /* Cap blk expansion to prevent dependent row overflow */
        var minAfter = 0;
    ...
    var onDocUp = function () {
      ...
      drawSVG();
      /* Fire custom event for version auto-snapshot */
    ...
    /* Expose for version management: capture current state as MD */
  ...
  function bindCutover(containerId) {
    ...
    C.querySelectorAll('.gt-cutover').forEach(function (cut) {
      ...
      var ORIG = 14, INIT = parseInt(cut.dataset.init || '14'), cur = INIT, cdrag = null;
      ...
      function apply(nc) {
        ...
        cut.querySelector('span').textContent = '割接 W' + (nc - 2) + '';
        rows.forEach(function (row) {
      ...
      var onCutUp = function () {
        ...
        document.body.style.userSelect = '';
        document.body.style.webkitUserSelect = '';
        cut.querySelector('span').textContent = '割接 W' + (cur - 2) + '';
      };
  ...
  function captureState(containerId, originalMD) {
    ...
    /* Capture current cutover position from DOM */
  ...
  /* ── Cleanup registry for document-level listeners ── */
  var _cleanup = {};

  /* ── 完整初始化入口 ── */
  function init(mdString, containerId) {
    /* Clean up previous bindings if re-initializing */
    if (_cleanup[containerId]) {
